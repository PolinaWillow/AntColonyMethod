...
namespace AntColonyMethod2
{
    public class AgentGroupM2
    {
        ...
        /// Список агентов
        /// </summary>
        public List<AgentM2> Agents { get; set; }
        ...
        public string AddNewAgent()
        {
            ...
            AgentM2 agent = new AgentM2 { idAgent = id };
            Agents.Add(agent);
        ...
        public void AddWayAgent(int[] wayAgent, string id)
        {
            ...
            AgentM2 agent = Agents.FirstOrDefault(r => r.idAgent == id);
            if (agent != null)
            {
        ...
        /// <param name="id">Id агента</param>
        /// <returns></returns>
        public AgentM2 FindAgent(string id)
        {
            Monitor.Enter(_monitor);
        ...
        /// <returns></returns>
        public int PheromoneEvaporation(List<AgentM2> agents, DataTask dataTask) //Испарение феромона
        {
            ...
            foreach (GraphParams graphElem in dataTask.graphWorkCopy.Params)
            {
                ...
                foreach (AgentM2 agentElem in agents)
                {
